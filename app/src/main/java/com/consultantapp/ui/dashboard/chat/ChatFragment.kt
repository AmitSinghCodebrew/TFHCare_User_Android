...
class ChatFragment : DaggerFragment() {
    ...
    private fun refreshChatLogs(
        ...
        requireActivity().runOnUiThread {
            val index = items.indexOf(items.find {
                it.to_user?.id == userId && it.id == requestChatId
            })
            if (index == -1) {
                getListing(true)
            } else {
    ...
    private fun registerReceiver() {
        if (!isReceiverRegistered) {
            ...
            intentFilter.addAction(PushType.CHAT)
    ...
    private val refreshRequests = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            when (intent.action) {
                PushType.REQUEST_COMPLETED, PushType.COMPLETED, PushType.CHAT_STARTED, PushType.CHAT -> {
                    getListing(true)
